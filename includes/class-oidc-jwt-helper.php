...
class OIDC_JWT_Helper {
	...
	public static function parse_jwt( $jwt ) {
		$parts = explode( '.', $jwt );
		if ( 3 !== count( $parts ) ) {
			return new WP_Error( 'invalid_jwt_format', __( 'Ungültiges JWT-Format.', 'jrtools-openid-connect' ) );
		}
		...
		if ( false === $header_json || false === $payload_json ) {
			return new WP_Error( 'jwt_decode_failed', __( 'JWT konnte nicht dekodiert werden.', 'jrtools-openid-connect' ) );
		}
		...
		if ( ! is_array( $header ) || ! is_array( $claims ) ) {
			return new WP_Error( 'jwt_parse_failed', __( 'JWT enthält kein gültiges JSON.', 'jrtools-openid-connect' ) );
		}
	...
	public static function verify_signature( $parts, $header, $jwks_uri ) {
		if ( ! function_exists( 'openssl_verify' ) ) {
			return new WP_Error( 'openssl_missing', __( 'PHP OpenSSL-Extension ist nicht verfügbar.', 'jrtools-openid-connect' ) );
		}
		...
		if ( 'RS256' !== $alg ) {
			return new WP_Error(
				...
				sprintf( __( 'Nicht unterstützter Signaturalgorithmus: %s', 'jrtools-openid-connect' ), sanitize_text_field( $alg ) )
			);
		}
		...
		if ( null === $jwk ) {
			delete_transient( 'jrtools_oidc_jwks_' . md5( $jwks_uri ) ); // phpcs:ignore -- md5 used as cache key, not for security
			$jwks = OIDC_JWK_Helper::get_jwks( $jwks_uri );
			if ( is_wp_error( $jwks ) ) {
				return $jwks;
			}
		...
		if ( null === $jwk ) {
			return new WP_Error( 'jwk_not_found', __( 'Passender Public Key im JWKS nicht gefunden.', 'jrtools-openid-connect' ) );
		}
		...
		if ( false === $signature_raw ) {
			return new WP_Error( 'sig_decode_failed', __( 'JWT-Signatur konnte nicht dekodiert werden.', 'jrtools-openid-connect' ) );
		}

		$public_key = openssl_pkey_get_public( $pem );
		if ( false === $public_key ) {
			return new WP_Error( 'pem_invalid', __( 'Public Key konnte nicht geladen werden.', 'jrtools-openid-connect' ) );
		}

		$result = openssl_verify( $signing_input, $signature_raw, $public_key, OPENSSL_ALGO_SHA256 );

		if ( 1 !== $result ) {
			return new WP_Error( 'sig_invalid', __( 'JWT-Signatur ist ungültig.', 'jrtools-openid-connect' ) );
		}
